...
packages_input=$(yad --width=500 --height=500 --title="Package Installation" --form --field="Enter additional packages to install::TXT" --field="Install:FBTN" "bash -c \"echo %1 >> $pkglist_file && zenity --info --text='Packages added to list'\"" --field="Cancel:FBTN" "bash -c \"zenity --info --text='Operation cancelled'\"" --field="View AUR:FBTN" "xdg-open https://aur.archlinux.org" --separator="," --borders=10)
